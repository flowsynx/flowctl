...
internal class WorkflowExecutionCancelCommandOptionsHandler : ICommandOptionsHandler<WorkflowExecutionCancelCommandOptions>
{
    private readonly IFlowCtlLogger _flowCtlLogger;
    private readonly IFlowSynxClient _flowSynxClient;
    private readonly IAuthenticationManager _authenticationManager;

    public WorkflowExecutionCancelCommandOptionsHandler(IFlowCtlLogger flowCtlLogger,
    ...
    public async Task<int> HandleAsync(WorkflowExecutionCancelCommandOptions options, CancellationToken cancellationToken)
    {
        await Execute(options, cancellationToken);
        return 0;
    }

    private async Task Execute(WorkflowExecutionCancelCommandOptions options, CancellationToken cancellationToken)
    {
        try
        {
