...
internal class WorkflowDetailsCommandOptionsHandler : ICommandOptionsHandler<WorkflowDetailsCommandOptions>
{
    private readonly IFlowCtlLogger _flowCtlLogger;
    private readonly IFlowSynxClient _flowSynxClient;
    private readonly IAuthenticationManager _authenticationManager;

    public WorkflowDetailsCommandOptionsHandler(IFlowCtlLogger flowCtlLogger,
    ...
    public async Task<int> HandleAsync(WorkflowDetailsCommandOptions options, CancellationToken cancellationToken)
    {
        await Execute(options, cancellationToken);
        return 0;
    }

    private async Task Execute(WorkflowDetailsCommandOptions options, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var result = await _flowSynxClient.WorkflowDetails(request, cancellationToken);
